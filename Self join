...
--   so that the services between 'Craiglockhart' and 'London Road' are shown.
...
SELECT DISTINCT a.num, a.company, bstop.name, c.num, c.company 
...
JOIN stops cstop ON (c.stop = cstop.id)
JOIN stops dstop ON (d.stop = dstop.id)
